...
class JsonLdContextNormalized {
  ...
  expandTerm(term, expandVocab, options = exports.defaultExpandOptions) {
    ...
    if (
      contextValue === null || (contextValue && contextValue["@id"] === null)
    ) {
      return null;
    }
    // Check the @id
    ...
    if (contextValue && expandVocab) {
      ...
      if (value && value !== term) {
        if (
          typeof value !== "string" ||
          (!Util_1.Util.isValidIri(value) && !Util_1.Util.isValidKeyword(value))
        ) {
    ...
    if (prefix) {
      ...
      if (value) {
        if (
          typeof contextPrefixValue === "string" || !options.allowPrefixForcing
        ) {
          ...
          if (
            value[0] !== "_" && !potentialKeyword &&
            !contextPrefixValue["@prefix"] && !(term in this.contextRaw)
          ) {
            // Treat the term as an absolute IRI
            return term;
          }
      ...
      expandVocab &&
      ((vocab || vocab === "") ||
        (options.allowVocabRelativeToBase && (base && vocabRelative))) &&
      !potentialKeyword && !Util_1.Util.isCompactIri(term)
    ) {
      if (vocabRelative) {
        if (options.allowVocabRelativeToBase) {
          return ((vocab || base)
            ? (0, relative_to_absolute_iri_1.resolve)(vocab, base)
            : "") + term;
        } else {
          throw new ErrorCoded_1.ErrorCoded(
            `Relative vocab expansion for term '${term}' with vocab '${vocab}' is not allowed.`,
      ...
      !expandVocab && base && !potentialKeyword &&
      !Util_1.Util.isCompactIri(term)
    ) {
    ...
    if (validIriMapping) {
      ...
      throw new ErrorCoded_1.ErrorCoded(
        `Invalid IRI mapping found for context entry '${term}': '${
  ...
  compactIri(iri, vocab) {
    ...
    if (
      vocab && this.contextRaw["@vocab"] &&
      iri.startsWith(this.contextRaw["@vocab"])
    ) {
      return iri.substr(this.contextRaw["@vocab"].length);
    }
    // Try @base compacting
    if (
      !vocab && this.contextRaw["@base"] &&
      iri.startsWith(this.contextRaw["@base"])
    ) {
